...
export default function BookAppointment() {
  ...
  const handleSubmit = async () => {
    const res = await fetch("http://localhost:5019/api/customers/appointments", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        customerId,
        appointmentAtUtc: new Date(date).toISOString(),
        notes,
      }),
    });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f9fafb", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#fff", padding: "40px", borderRadius: "12px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)", width: "100%", maxWidth: "480px" }}>
        <h1 style={{ fontSize: "24px", fontWeight: "700", marginBottom: "8px", color: "#111" }}>Book Appointment</h1>
        <p style={{ color: "#6b7280", marginBottom: "24px" }}>Schedule a service appointment</p>
        ...
        <label style={{ display: "block", marginBottom: "6px", fontWeight: "500", color: "#374151" }}>Appointment Date & Time</label>
